...
const CreateNewUser = async (record) => {
  // Pre-validation
  const existsUser = await db.afs_users.findOne({
    where: {
      email: record.email,
    },
  });
  ...
  const existsUsername = await db.afs_logins.findOne({
    where: {
  ...
  const role = await db.afs_master_roles.findOne({ where: { id: record.role_id } });
  if (role == null) throw new Error('Role does not exists');


  // Create user in the "users" table
  const userRes = await db.afs_users.create({
  ...
  await db.afs_user_role_mappings.create({
    user_id: userRes.dataValues.id,
    role_id: role.id,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
...
const getUserById = async (id) => {
  const user = await db.afs_users.findOne({ where: { id } });
  if (!user) {
